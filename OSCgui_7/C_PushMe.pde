...
class PushMe {
  ...
  //Constructor2////////////////////////////
  PushMe(int aix, int ax, int ay, float aw, float ah, int amode, String albl, int aclrnum) {
    ix = aix;
    x = ax;
    y = ay;
    mode = amode;
  ...
  void drw() {
    ...
    noStroke();
    rectMode(CORNER);
    if (pressed==1) fill(255, 255, 0);
    else fill(clr.getByIx(clrnum));
    ////Has Focus//////////////
  ...
  void msprs() {
    if (mode==0)pressed=1;
    else pressed = (pressed+1)%2;
    if (msggate) {
      osc.send("/"+lbl, new Object[]{groupId, ix, lbl, pressed}, sc);
      msggate = false;
    }
  }//end msprs
  //msrel
  void msrel() {
    if (!msggate) {
      if (mode==0)osc.send("/"+lbl, new Object[]{groupId, ix, lbl, 0}, sc);
      msggate = true;
    }
  }//end msrel
}//end class PushMe
...
class PushMeSet {
  ...
  void mk2(int ix, int x, int y, float w, float h, int mode, String lbl, int clrnum) {
    cset.add( new PushMe(ix, x, y, w, h, mode, lbl, clrnum) );
  } //end mk2 method
